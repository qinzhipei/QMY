...
dt3$betaX <- coefficients(res.Cox)[names(coefficients(res.Cox)) == "alcohol"] * (dt3$alcohol-mean(dt3$alcoholdavaZC)
...
#0428
data424 <- read.table(file=r'{E:\QMY\result\Data428.csv}', 
                      header=TRUE, sep=",")
data_0 = data424[which(data424$sex == 0),] 
data_1 = data424[which(data424$sex == 1),] 
res.Cox <- coxph(Surv(Duration_ukb, termi)~
                   minimum_avg+alpha_avg+beta_avg+
                 acrotime_avg+F_pseudo_avg+UpMesor_avg+metabolic_syd+
                   age+smoke+alcohol+ethnic_x+season_sin+season_cos, data = data424)
summary(res.Cox)
cox.zph(res.Cox)

res.Cox0 <- coxph(Surv(Duration_ukb, termi)~
                    minimum_avg+alpha_avg+beta_avg+
                    acrotime_avg+F_pseudo_avg+UpMesor_avg+metabolic_syd+
                    age+smoke+alcohol+ethnic_x+season_sin+season_cos, data = data_0)
summary(res.Cox0)
cox.zph(res.Cox0)

res.Cox1 <- coxph(Surv(Duration_ukb, termi)~
                    minimum_avg+amp_avg+alpha_avg+beta_avg+
                    acrotime_avg+F_pseudo_avg+UpMesor_avg+metabolic_syd+
                    age+smoke+alcohol+ethnic_x+season_sin+season_cos, data = data_1)
summary(res.Cox1)
cox.zph(res.Cox1)
